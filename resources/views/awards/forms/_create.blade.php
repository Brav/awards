...
<form
    ...
    <input type="hidden" id="background-award"
        name="background-award" value="{{ $background ? $background->award : null }}">

    <input type="hidden" id="background-winner"
        name="background-winner" value="{{ $background ? $background->winner : null }}">

    <input type="hidden" id="background-logo"
        name="background-logo" value="{{ $background ? $background->logo : null }}">
    ...
    <div class="form-row align-items-center mb-4">
        <div class="form-group col-md-12">
            ...
            value=""
            rows="4"
            placeholder="1 x overall annual winner">{{ old('award-footer-info') }}</textarea>
        </div>
        ...
        <div class="col">
            <div class="form-check">
                <input class="form-check-input"
                       ...
                       id="graduated_year"
                       name="graduated_year"
                       @if (old('graduated_year'))
                           checked
                    @endif>
                <label class="form-check-label" for="graduated_year">Graduated Year Field?</label>
            ...
            <div class="row">
                @foreach ($logos as $image)
                    @php
                        $data = pathinfo($image);
                    @endphp
                    <div class="col-lg-4 col-xl-2 background-image logo-category
                            @if ($background && ($background->logo === $data['basename']))
